...
class CI_Image_lib {
	...
	function initialize($props = array()) {
		...
		/*
		 * Is getimagesize() Available? We use it to determine the image properties (width/height). Note: We need to figure out how to determine image properties using ImageMagick and NetPBM
		 */
		...
		/*
		 * Set the full server path The source image may or may not contain a path. Either way, we'll try use realpath to generate the full server path in order to more reliably read it.
		 */
		...
		/*
		 * Assign the "new" image name/path If the user has set a "new_image" name it means we are making a copy of the source image. If not it means we are altering the original. We'll set the destination filename and path accordingly.
		 */
		if ($this->new_image == '') {
		...
		/*
		 * Compile the finalized filenames/paths We'll create two master strings containing the full server path to the source image and the full server path to the destination image. We'll also split the destination image name so we can insert the thumbnail marker if needed.
		 */
		...
		/*
		 * Should we maintain image proportions? When creating thumbs or copies, the target width/height might not be in correct proportion with the source image's width/height. We'll recalculate it here.
		 */
		...
		/*
		 * Was a width and height specified? If the destination width/height was not submitted we will use the values from the actual file
		 */
		if ($this->width == '')
